...
void CountLowDimu(int run1=326381, int run2=326452, bool add=true, TString listn="runlist_2018PbPb_20181109") {
		  ...
		  TH1F *htot = (TH1F*)finini->Get(Form("DQMData/Run %i/Muons/Run summary/diMuonHistograms/GlbGlbMuon_LM",runini));
////////
...
//		  while (!flist.eof()) { //needed }
		  ...
		  //###TH1F *h = (TH1F*) file->Get("Run summary/DiMuonHistograms/GlbGlbMuon_LM");
		  TH1F *h = (TH1F*) file->Get(Form("DQMData/Run %i/Muons/Run summary/diMuonHistograms/GlbGlbMuon_LM",run));
		  ...
		  tl_body->DrawLatexNDC(0.18,0.75,Form("%i Dimuons (from counting)",justcount));
		  tl_body->DrawLatexNDC(0.18,0.71,Form("%i Dimuons (from fitting)",integral));
		  tl_body->DrawLatexNDC(0.18,0.67,Form("(#sigma = %.1f nb)",integral/lumi));
		  c1->SaveAs(Form("figures_runbyrun/plotLM_%i.pdf",run));
		  c1->SaveAs(Form("figures_runbyrun/plotLM_%i.png",run));

{
		  ...
		  //h->SetAxisRange(0.0, h->GetMaximum()*1.5, "Y");
		  htot->SetAxisRange(0.0, 1500.0, "Y");
		  ...
		  tl_bodyadd->DrawLatexNDC(0.18,0.75,Form("%i Dimuons (from counting)",justcountadd));
		  tl_bodyadd->DrawLatexNDC(0.18,0.71,Form("%i Dimuons (from fitting)",integraladd));
		  tl_bodyadd->DrawLatexNDC(0.18,0.67,Form("(#sigma = %.1f nb)",integraladd/lumiadd));
	  	  if (add) c1->SaveAs("figures_wholerun/plotLMadd.gif+100");


		  c1->SaveAs(Form("figures_wholerun/plotLM_%i_%i.pdf",run1,run));
		  c1->SaveAs(Form("figures_wholerun/plotLM_%i_%i.png",run1,run));
